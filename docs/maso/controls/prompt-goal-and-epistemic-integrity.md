...
|---------|-------------|---------------------|
| **PG-1.1** Input sanitisation per agent | Every agent applies input guardrails that detect and block known prompt injection patterns | Guardrails operate on all input sources: user input, inter-agent messages, RAG content, tool outputs, and any files or configuration the agent reads at runtime. Configuration files (`CLAUDE.md`, `.cursorrules`, `AGENTS.md`, system prompts loaded from the filesystem) are instruction sources and must be treated as untrusted input when loaded from repositories or shared directories not under the operator's direct control. The TrapDoor supply chain campaign (May 2026) confirmed this attack class in production using zero-width Unicode encoding to hide instructions from human reviewers. |
